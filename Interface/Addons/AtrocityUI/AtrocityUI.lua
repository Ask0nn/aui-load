...
local pages = {
	...
	["WarpDeplete"] = function(shouldImport, forceReImport, resolution)
		return function()
			...
			if forceReImport then
				PluginInstallFrame.Desc2:SetText(Red(string.format("This will import WarpDeplete %s.", GetAddonInstalledVersion("WarpDeplete"))))
			elseif shouldImport then
				PluginInstallFrame.Desc2:SetText(string.format("Your version of WarpDeplete is %s but the latest is %s. The below button will update the WarpDeplete profile and load it.", Red(GetAddonSavedVersion("WarpDeplete")), Green(GetAddonInstalledVersion("WarpDeplete"))))
			else
				PluginInstallFrame.Desc2:SetText(string.format("Your version of WarpDeplete is %s, which is the latest. The below button will just load the profile.", Green(GetAddonSavedVersion("WarpDeplete"))))
			end
			PluginInstallFrame.Option1:Show()
			PluginInstallFrame.Option1:SetScript("OnClick", function() atrocityUIData:SetWarpDeplete(resolution, shouldImport) end)
...
function RunAtrocityUI(msg, _)
	...
	elseif (tokens[1] == "ver") or (tokens[1] == "version") or (tokens[1] == "v") then
		print(string.format("|cffffffffAtrocityUI version %s", GetAddonVersion()))
	else
